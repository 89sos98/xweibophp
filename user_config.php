...
if (XWB_SERVER_ENV_TYPE!=='sae'){
	/// 产品安装路径（w_base_url_path）
	define('W_BASE_URL_PATH','/');
	/// 微博 APP_KEY
	define('WB_AKEY','');
	/// 微博 SECRET_KEY
	define('WB_SKEY','');
	/// 官方微博功能中创建list使用的ID（system_sina_uid）
...
//----------------------------------------------------------------------
/// HTTP适配器选择配置  fsockopen curl
define('HTTP_ADAPTER','fsockopen');
/// CACHE适配器选择配置 file serialize memcache
...
//----------------------------------------------------------------------
/// xwb 插件通讯的api签名认证key（xplugin_api_key）
