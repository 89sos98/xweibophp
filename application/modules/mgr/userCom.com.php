...
class userCom {
	...
	*/
	...
	/**
	* 根据一组sina_uid称获取用户昵称
    * @param array $sina_uid
    * @return RST_Array
	*/
	function getNicknameByUids($sina_uids) {
		$db = APP :: ADP('db');
		$find = array();
		foreach($sina_uids as $uid){
			if(is_numeric($uid)){
				$find[] = "'". $db->escape($uid). "'";
			}
		}
		...
		$where = " WHERE `sina_uid` IN (" . implode(',', $find) . ")"; 
		$sql = 'SELECT `sina_uid`, `nickname` FROM ' . $db->getPrefix() . T_USERS. $where . ' ORDER BY `sina_uid` DESC ';
		$res = $db->query($sql);
		if(!is_array($res)){
			return RST(array());
		}else{
