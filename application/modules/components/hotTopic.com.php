...
//		$this->clearCfgCache();
		...
		if ($topic_get == 0 && $topics != NULL && is_array($topics) && isset($topics[0])) {
			...
			foreach($topics as $index => $topic){
				$url = empty($param['tid'][$index]) ? 
							URL('search.weibo', array('base_app' => $base_app, 'k' =>F('escape',$topic))) :
							URL('custom', array('page_id'=>$param['tid'][$index]));
				$rs[] = array('topic'=>$topic,'query'=> $url);
			}
			return RST($rs);
		} else { //否则使用新浪API
			if($topic_get == 2) {
			...
			if ($rs['errno'] == 0) {
				...
				foreach (current($rs['rst']['trends']) as $row) {
					...
					array_push($tmp, array('topic' => $row['name'], 'query' => URL('search.weibo', array('base_app' => $base_app, 'k' =>$row['query']))));
				}
