...
class output_mod
{
	...
	function default_action()
	{
		...
		/// 内容单元的宽度
		$width = intval(V('g:width'));
		/// 内容单元的高度
		$height = intval(V('g:height', 550));
		/// 是否显示标题
		$show_title = (bool)(V('g:show_title', false));
		/// 是否显示边框
		$show_border = (bool)(V('g:show_border', false));
		/// 是否显示logo
		$show_logo = (bool)(V('g:show_logo', false));
		/// 皮肤
		$skin = intval(V('g:skin', 1));
		if ($skin % 10 != 0) {
			$skin = '0'.$skin;
		}
			...
			case 4:
				...
				if(!empty($batch_info)&&isset($batch_info['rst'])&&is_array($batch_info['rst'])){
					...
					foreach($batch_info['rst'] as $row){
						$k = array_search($row['id'], $ids);
						if($k !== false){
							$rst[$k]['description']=$row['description'];
						}
					}
				}
