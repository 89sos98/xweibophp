...
class wb_live_mod extends action {
	...
	function saveLive(){
		if(strtolower(V('s:REQUEST_METHOD'))=='post'){
			...
			$s = preg_replace("#<param\s[^>]*name=\"wmode\"[^>]*>#sim","",$s);
			$s = preg_replace("#(<embed\s[^>]*)wmode=\"opaque\"([^>]*>)#sim","\\1	\\2",$s);
			$s = preg_replace("#<embed\s#sim","<embed	wmode=\"opaque\"	", $s);
			$s = preg_replace("#(>|\s)</object>#sim","\\1\n<param value=\"transparent\" name=\"wmode\">\n</object>", $s);
