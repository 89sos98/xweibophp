...
?>
	<?php 
		...
		if (isset($retweeted_status)): 
			...
			//没有source(该微博可能被删除)、或者deleted标为1(该微博可能被原作者删除)，就认为API给出的是删除数据	
			}elseif(!isset($retweeted_status['source']) || (isset($retweeted_status['deleted']) && 1 == $retweeted_status['deleted'])){
				$isFilter = true;
				$retweeted_status['deleted'] = 1;
				$errmsg = isset($retweeted_status['text']) ? F('escape', $retweeted_status['text']) : 'WEIBO_NOT_EXIST';
			} else {
				//原微博内容
				$rt = &$retweeted_status;
	...
	<?php if ((!isset($show_delete_link) || true == $show_delete_link) && $uid == $user['id']):?><a href="#" rel="e:dl"><?php LO('modules__feed__delete');?></a>|<?php endif;?>
	<?php if(!isset($retweeted_status['deleted'])): ?><a href="#" rel="e:fw" id="fw"><?php LO('modules__feed__repost'); echo isset($repostCnt) ? "($repostCnt)" : ''; ?></a>|<?php endif; ?>
	<?php $favorited=isset($favorited) ? $favorited : false; if($favorited):?><a href="#" rel="e:ufr,t:1"><?php LO('modules__feed__unfav');?></a><?php else: ?><a href="#" rel="e:fr"><?php LO('modules__feed__fav');?></a><?php endif; ?>|
	<a href="javascript:;"<?php if (!$disable_comment):?> rel="e:cm"<?php endif;?> id="cm"><?php LO('modules__feed__comment');echo isset($commentCnt) ? "($commentCnt)" : ''; ?></a>
