<?php
$disallow_not_friend_local_pm = 1;
if (USER::isUserLogin() && USER::uid() != $userinfo['id']) {
	...
	//该人是否不是本站用户、或者是否设置了禁止我没有关注的人发送私信
	if(USER::uid() > 0 && 1 == $userinfo['is_localsite_user']){
		$disallow_not_friend_local_pm = intval(DS('common/userConfig.get', null, 'disallow_not_friend_local_pm', $userinfo['id']));
	}
			...
			<?php if (USER::isUserLogin()):?>
				<?php if (!$blocked):?>
					<span class="opera-area-r">
						<?php if ( (HAS_DIRECT_MESSAGES && $followedBack) || (!HAS_DIRECT_MESSAGES && 0 == $disallow_not_friend_local_pm) ):?>
					...
					<?php if (!$followed):?>
					<a href="#" rel="e:fl,t:3" class="btn-s4"><span><s class="plus">+</s><?php LO('common__template__toFollow');?></span></a>
