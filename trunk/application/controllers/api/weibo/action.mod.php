...
class action_mod
{
	...
	/**
	 * SERVER API：设置ip白名单访问，默认不使用ip白名单
	 * 如果要ip白名单, 需要手动填写ip填写,多个ip地址用逗号隔开
	 * 比如：array('192.168.1.100', '192.168.1.101');
	 * @var array
	 */
	var $_srvapi_access_ips = array();
	
	/**
	 * SERVER API：是否使用匿名身份(有接口限制)
	 * @var array
	 */
	var $_srvapi_allow_anonymous = array();

	/**
	 * SERVER API：是否使用系统默认身份(有接口限制)
	 * @var array
	 */
	var $_srvapi_allow_others = array();
	...
	function action_mod()
	{
		...
		if (IS_IN_API_REQUEST) {
			/// ip白名单判断
			if (!empty($this->_srvapi_access_ips)) {
				$ip = F('get_client_ip');
				if (!in_array($ip, $this->_srvapi_access_ips)) {
			...
			$db = APP :: ADP('db');
			$result = array();
			if(is_numeric($api_uid) && $api_uid > 0){
				...
				(isset($result['token_secret']) && empty($result['token_secret']))) 
			{
				...
				if (empty($this->_srvapi_allow_anonymous) && empty($this->_srvapi_allow_others)) {
					die(json_encode(RST('', '3010003', 'Does not bind the user')));
				} elseif (in_array($m['function'], $this->_srvapi_allow_others)) {
					DR('xweibo/xwb.setToken', false, 2);
				} elseif (!in_array($m['function'], $this->_srvapi_allow_anonymous)) {
					die(json_encode(RST('', '3010003', 'Does not bind the user')));
				}
			} else {
				DR('xweibo/xwb.setToken', false, 3, $result['access_token'], $result['token_secret']);
				USER::uid($result['sina_uid']);
			}
		} elseif (IS_IN_JS_REQUEST) {
			$m = APP::getRequestRoute(true);
			if(!in_array($m['function'], $this->_jsapi_ignore_sec_check) && 0 != XSec::checkReferer()){
				APP::ajaxRst(null, 1040017, 'XWEIBO JS API SECURITY CHECK FAILURE');
				exit();
			}
		}else{
	...
	*/
	...
	function reply()
	{
		...
		$add = false;
		/// 调用回复评论接口
		$result = DS('xweibo/xwb.reply', '', $id, $cid, $text, 1);
		$result = F('weibo_filter', $result, true);
	...
	function sendDirectMessage()
	{
		if(HAS_DIRECT_MESSAGES){
			$this->_sendDirectMessage_API();
		}else{
			$this->_sendDirectMessage_localpm();
		}
	}
	
	/**
	 * 发送一条私信(保存在新浪微博上的私信，API方式发送)
	 */
	function _sendDirectMessage_API()
	{
		$id = V('p:id');
		$name = V('p:name');
		$text = V('p:text');
	 ...
	 * 发送一条私信（本地私信系统）
	 */
	function _sendDirectMessage_localpm()
	{
		$id = V('p:id');
		$name = V('p:name');
		$text = V('p:text');
		...
		if(!empty($name)){
			$result = $srv->writeNewUsingNickname(USER::uid(), $name, $text);
		}elseif(!empty($id)){
			$result = $srv->writeNew(USER::uid(), $id, $text);
		}
		
		if(true == $result['rst']){
			/// 上报
			F('report', 'letter_local', 'http');
		}
		
		APP::ajaxRst($result['rst'], $result['errno'], $result['err']);
		exit;
	}	
	...
	function deleteDirectMessage()
	{
		...
		if(HAS_DIRECT_MESSAGES){
			$this->_deleteDirectMessage_API($id);
			
		}else{
			$this->_deleteDirectMessage_localpm($id);
		}		
	...
	/**
	 * 删除一条私信(保存在新浪微博上的私信，API方式删除)
	 */
	function _deleteDirectMessage_API($id)
	{
	 ...
	 * 删除一条私信（本地私信系统）
	 */
	function _deleteDirectMessage_localpm($id)
	{
		$srv = APP::O('localpm_service');
		$srv->destroyByid($id, USER::uid());
	}	
	
	/**
	 * 批量删除私信
	 */
	function deleteDirectMessageBatch(){
		if(!HAS_DIRECT_MESSAGES){
			$this->_deleteDirectMessageBatch_localpm();
		}else{
			$this->_deleteDirectMessageBatch_API();
		}
	}
	 ...
	 * 暂时没有实现
	 */
	function _deleteDirectMessageBatch_API(){
		APP::ajaxrst(true);
	}
	 ...
	 * 目前仅能删除当前用户与某个人的所有私信
	 */
	function _deleteDirectMessageBatch_localpm(){
		$uid = V('p:uid');
		if(is_numeric($uid) && $uid > 0){
			$srv = APP::O('localpm_service');
			$srv->destroyBatchBySinaUid(USER::uid(), $uid);
		}
		APP::ajaxrst(true);
	}
	...
	function unread()
	{
		...
		if(HAS_DIRECT_MESSAGES){
			...
			//开启本地私信的情况下，采用本地私信设置提醒
			$localpm_alert_new = intval(V('-:userConfig/localpm_alert_new', 1)) == 1 ? 1 : 0;
			if(1 == $localpm_alert_new){
				$result['dm'] = intval(V('-:userConfig/new_local_pm'));
			}else{
				$result['dm'] = 0;
			}
		}
	...
	function saveNotice()
	{
		...
		//开启本地私信的情况下，不更新该api设置，只采取本地设置覆盖措施
		if(HAS_DIRECT_MESSAGES){
			$notice['dm'] = intval(V('p:dm', 0));
		}else{
			$local_setting['localpm_alert_new'] = intval(V('p:dm', 0));
		}
		/// 更新显示tip方式设置
		$local_setting['user_newfeed'] = (int)V('p:newfeed');
		$local_setting['user_newnotice'] = V('p:notice', 0);
	 ...
	 // @param string $type 1--评论数，2--@数，3--私信数，4--关注我的数
		...
		*/
		...
		DS('common/userConfig.set', '', $local_setting, null);
    ...
    */
	 ...
	 * 更新当前登录的本地私信用户设置
	 */
	function saveLocalpmSet(){
		$disallow = (1 == intval(V('p:disallow', 0))) ? 1 : 0;
		DS('common/userConfig.set', null, 'disallow_not_friend_local_pm', $disallow, USER::uid());
		APP::ajaxRst(true, 0);
	}
