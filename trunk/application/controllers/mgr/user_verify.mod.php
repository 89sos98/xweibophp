	...
	*/
	...
	function webAuthenWay() {
		if ($this->_isPost()) {
			...
			$authen_request_email = V('p:authen_request_email','');
			
			if(!empty($authen_request_email) && !preg_match('/^[\w\-\.]+@[\w\-\.]+(\.\w+)+$/', $authen_request_email)){
				$this->_error('E-MAIL格式不正确！', array('webAuthenWay'));
			}
			...
			$rs = DR('common/sysConfig.set', '', 'authen_request_email', $authen_request_email);	//设置请求认证的E-mail
