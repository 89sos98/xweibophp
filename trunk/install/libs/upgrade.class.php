...
class upgrade {
	 ...
	 *
	 */
	function action_db22($db_prefix) 
	{
		global $_LANG;
		///更新表结构
		upgrade_tables($db_prefix, $this->link);
		
		$tb_pages = $db_prefix.'pages';
		
		$check_sql = array(
			'type' => "SHOW COLUMNS FROM `{$tb_pages}` LIKE 'type'",
			'params' => "SHOW COLUMNS FROM `{$tb_pages}` LIKE 'param'",
		);
		
		$upg_col_sql = array(
			'type' => "ALTER TABLE `{$tb_pages}` ADD COLUMN `type` int(11) default '0' COMMENT '页面类型：专题、普通'",
			'params' => "ALTER TABLE `{$tb_pages}` ADD COLUMN `params` text",
		);
		
		//检查表字段
		$need_upg_col = array();
		foreach($check_sql as $col => $sql){
			$ret = mysql_query($sql, $this->link);
			if(false === $ret){
				install_log('sql: '.$sql." \r\nerrno: ".mysql_errno($this->link)." \r\nerror: ".mysql_error($this->link));
				show_msg($_LANG['upgrade_2.2_check_col_failure']);
			}
		...
		//升级表字段
		foreach($need_upg_col as $col){
			$ret = mysql_query($upg_col_sql[$col], $this->link);
			if(false === $ret){
				install_log('sql: '.$upg_col_sql[$col]." \r\nerrno: ".mysql_errno($this->link)." \r\nerror: ".mysql_error($this->link));
				show_msg($_LANG['upgrade_2.2_upgrade_col_failure']);
			}
		}
		
		//升级表的索引
		mysql_query("ALTER TABLE `{$db_prefix}component_users` ADD INDEX `group_id` (`group_id`)", $this->link);
		mysql_query("ALTER TABLE `{$db_prefix}celeb` ADD INDEX `c_id1` (`c_id1`, `c_id2`, `sort`)", $this->link);
		
		///更新sys_config wb_version版本号的值
		$sysConfigTable = $db_prefix.'sys_config';
		$sql = "UPDATE `$sysConfigTable` set `value` = '2.2' where `key` = 'wb_version'";
		$ret = mysql_query($sql, $this->link);
